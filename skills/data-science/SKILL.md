...
name: data-science
description: >
  Plan, execute, validate, and communicate rigorous data analysis from raw data through reproducible
  findings. Use for statistical questions, experiments, forecasting, causal inference, modeling,
  anomaly detection, segmentation, or any request to draw conclusions from data.
---
...
Understand the problem before writing code. If the request does not already establish the question,
data, stakes, and deliverable, ask one focused question at a time. Use the platform's structured
question interface when available; otherwise ask directly. Prefer concrete choices when helpful.
